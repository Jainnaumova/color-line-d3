...
function App() {
  ...
  const [coords, setCoords] = useState({ xAxios: null, yAxios: null })
  ...
  const updateChart = (xScale, yScale) => {
  ...
  const handleTest = (svg, xScale, yScale) => {
    ...
    const x = svg.select("#bannerLine").attr("x1");
    console.log("XXX", x);
    const y = svg.select("#bannerLine").attr("y1");
    console.log("YYY", y);
    // const x = window.innerWidth - 100;
    // this.setState({ x: xScale(x), y: yScale(y) });
    setCoords({ ...coords, xAxios: +x + +a, yAxios: y });
  ...
  return (
    <div className="App">
      <div id="chart" />
      <Banner bannerData={data} left={coords.xAxios} top={coords.yAxios} />
    </div>
  );
}
